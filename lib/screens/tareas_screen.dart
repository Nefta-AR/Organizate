...
class _TareasScreenState extends State<TareasScreen> {
  ...
  static const _filters = [
    _CategoryFilter(label: 'Todas',    value: null,        icon: Icons.apps_rounded,      color: Colors.blueGrey),
    _CategoryFilter(label: 'Estudios', value: 'Estudios',  icon: Icons.menu_book,         color: Colors.orange),
    _CategoryFilter(label: 'Hogar',    value: 'Hogar',     icon: Icons.cottage,           color: Colors.green),
    _CategoryFilter(label: 'Meds',     value: 'Meds',      icon: Icons.medication,        color: Colors.red),
    _CategoryFilter(label: 'Foco',     value: 'Foco',      icon: Icons.self_improvement,  color: Colors.purple),
    _CategoryFilter(label: 'General',  value: 'General',   icon: Icons.task_alt,          color: Colors.blueGrey),
  ];
        ...
        StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            return Row(children: [
              ...
              Text('$points', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.black87)),
              const SizedBox(width: 12),
              const Icon(Icons.local_fire_department, color: Colors.deepOrange, size: 20),
              const SizedBox(width: 2),
              Text('$streak', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.black87)),
              ...
              Padding(
                ...
                child: CircleAvatar(
                  ...
                  backgroundImage: avatar != null ? AssetImage('assets/avatars/$avatar.png') : null,
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isSelected ? f.color.withValues(alpha: 0.15) : Colors.grey.shade100,
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(f.icon, size: 14, color: isSelected ? f.color : Colors.grey.shade600),
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final filtered = _selectedCategory == null
            ? all
            : all.where((d) => d.data()['category'] == _selectedCategory).toList();

        // Pending first, then done; creation order preserved within each group
  ...
  Future<void> _toggleTask(String taskId, bool isDone) async {
    ...
    try {
      await batch.commit();
      if (!isDone) {
        await ReminderDispatcher.cancelTaskReminder(userDocRef: _userDocRef, taskId: taskId);
      ...
      builder: (dlgCtx) => StatefulBuilder(
        builder: (statCtx, setDlg) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              TextField(controller: ctrl, decoration: const InputDecoration(hintText: 'Descripción'), autofocus: true),
              const SizedBox(height: 16),
              ...
              Row(children: [
                Expanded(
                  child: Text(
                    selectedDate == null ? 'Sin fecha' : 'Entrega: ${_dateFormatter.format(selectedDate!)}',
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    final p = await pickDateTime(context: statCtx, initialDate: selectedDate);
                    if (p != null) setDlg(() => selectedDate = p);
                  },
                ),
                if (selectedDate != null)
                  IconButton(icon: const Icon(Icons.clear, size: 18), onPressed: () => setDlg(() => selectedDate = null)),
              ]),
              const SizedBox(height: 8),
              DropdownButtonFormField<int?>(
                key: ValueKey(selectedReminder),
                decoration: const InputDecoration(labelText: 'Recordatorio', border: OutlineInputBorder()),
                initialValue: selectedReminder,
                items: kReminderOptions.map((o) => DropdownMenuItem<int?>(value: o['minutes'] as int?, child: Text(o['label'] as String))).toList(),
                onChanged: (v) => setDlg(() => selectedReminder = v),
              ),
            ]),
          ),
          actions: [
            TextButton(onPressed: () => Navigator.of(dlgCtx).pop(), child: const Text('Cancelar')),
            ElevatedButton(
              onPressed: isSaving
                  ? null
                  : () async {
                      if (ctrl.text.trim().isEmpty || selectedCat == null) return;
                      ...
                      try {
                        final data = <String, dynamic>{
                          ...
                          if (selectedDate != null) 'dueDate': Timestamp.fromDate(selectedDate!),
                        };
                        final ref = await _tasksCollection.add(data);
                        await ReminderDispatcher.scheduleTaskReminder(
                          userDocRef: _userDocRef, taskId: ref.id,
                          taskTitle: ctrl.text.trim(), dueDate: selectedDate, reminderMinutes: selectedReminder,
                        );
  ...
  void _showTaskOptionsDialog(BuildContext ctx, String taskId, String text,
      String? category, Timestamp? dueDate, int? reminderMinutes) {
    showDialog(
      context: ctx,
      builder: (dlgCtx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ListTile(
            ...
            onTap: () {
              ...
              _showEditTaskDialog(ctx, taskId, text, category, dueDate, reminderMinutes);
            },
          ),
          ListTile(
        ...
        actions: [TextButton(onPressed: () => Navigator.of(dlgCtx).pop(), child: const Text('Cancelar'))],
      ),
    );
  }

  void _showEditTaskDialog(BuildContext ctx, String taskId, String currentText,
      String? currentCategory, Timestamp? currentDueDate, int? currentReminder) {
    final ctrl         = TextEditingController(text: currentText);
    final categories   = ['General', 'Estudios', 'Hogar', 'Meds', 'Foco'];
    String? selCat     = categories.contains(currentCategory) ? currentCategory : 'General';
    ...
    showDialog(
      ...
      builder: (dlgCtx) => StatefulBuilder(
        builder: (statCtx, setDlg) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              TextField(controller: ctrl, decoration: const InputDecoration(hintText: 'Nuevo texto'), autofocus: true),
              const SizedBox(height: 16),
              DropdownButtonFormField<String>(
                initialValue: selCat,
                decoration: const InputDecoration(labelText: 'Categoría'),
              ...
              Row(children: [
                Expanded(
                  child: Text(
                    selDate == null ? 'Sin fecha' : 'Entrega: ${_dateFormatter.format(selDate!)}',
                    style: TextStyle(color: Colors.grey.shade600, fontSize: 13),
                  ),
                ),
                IconButton(
                  icon: const Icon(Icons.calendar_today, size: 20),
                  onPressed: () async {
                    final p = await pickDateTime(context: statCtx, initialDate: selDate);
                    if (p != null) setDlg(() => selDate = p);
                  },
                ),
                if (selDate != null)
                  IconButton(icon: const Icon(Icons.clear, size: 18), onPressed: () => setDlg(() => selDate = null)),
              ]),
              const SizedBox(height: 8),
              DropdownButtonFormField<int?>(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                final nav = Navigator.of(dlgCtx);
                if (ctrl.text.trim().isEmpty || selCat == null) { nav.pop(); return; }
                try {
                  await _tasksCollection.doc(taskId).update({
                    ...
                    'dueDate':               selDate == null ? FieldValue.delete() : Timestamp.fromDate(selDate!),
                  });
                  await ReminderDispatcher.cancelTaskReminder(userDocRef: _userDocRef, taskId: taskId);
                  await ReminderDispatcher.scheduleTaskReminder(
                    userDocRef: _userDocRef, taskId: taskId,
                    taskTitle: ctrl.text.trim(), dueDate: selDate, reminderMinutes: selReminder,
                  );
                } catch (e) {
  ...
  Color _colorOf(String cat) {
    switch (cat) {
      case 'Estudios': return Colors.orange;
      case 'Hogar':    return Colors.green;
      case 'Meds':     return Colors.red;
      case 'Foco':     return Colors.purple;
  ...
  IconData _iconOf(String cat) {
    switch (cat) {
      case 'Estudios': return Icons.menu_book;
      case 'Hogar':    return Icons.cottage;
      case 'Meds':     return Icons.medication;
      case 'Foco':     return Icons.self_improvement;

...
class _MedsScreenState extends State<MedsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        backgroundColor: Colors.red, // <-- CAMBIADO: Color Rojo de Meds
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        stream: tasksCollection
            .where('category', isEqualTo: 'Meds') // <-- CAMBIADO: Filtro por 'Meds'
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) { 
            debugPrint('¡¡¡ERROR EN FIREBASE (MEDS): ${snapshot.error}!!!');
            return const Center(child: Text('Error al cargar tareas')); 
          }
          
          // Mensaje específico si no hay tareas de MEDS
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) { 
            return const Center(child: Padding( 
              padding: EdgeInsets.all(32.0), 
              child: Text('No tienes recordatorios de medicamentos.\n¡Añade uno con el botón +!', // <-- CAMBIADO
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                onLongPress: () => _showTaskOptionsDialog(context, taskId, currentText, 'Meds', currentDueDate), // <-- CAMBIADO
                child: _buildGoalItem(
                  icon: Icons.medication, // <-- CAMBIADO: Ícono de Meds
                  iconColor: Colors.red, // <-- CAMBIADO: Color de Meds
                  text: currentText,
                  ...
                  onDonePressed: () {
                      ...
                      batch.commit().catchError((error) { debugPrint("Error al actualizar: $error"); });
                  },
                ),
              );
            },
  ...
  // Diálogo para AÑADIR una nueva tarea (¡GUARDANDO "Meds"!)
  void _showAddTaskDialog(BuildContext context) {
    final TextEditingController taskController = TextEditingController();
    DateTime? selectedDueDate;
    const String fixedCategory = 'Meds'; // <-- CAMBIADO: Categoría fija
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              title: const Text('Nuevo Recordatorio (Meds)'), // <-- CAMBIADO: Título específico
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    // No necesitamos selector de categoría, ya es 'Meds'
                    Row(
                      children: [
                        Expanded(
                          child: Text(
                        ...
                        IconButton(
                          icon: const Icon(Icons.calendar_today, color: Colors.red), // <-- CAMBIADO: Color de Meds
                          onPressed: () async {
                  ...
                  onPressed: () {
                    if (taskController.text.isNotEmpty) {
                      ...
                      Map<String, dynamic> taskData = {
                        ...
                        'category': fixedCategory, // <-- Guarda "Meds"
                        'iconName': iconName,
                        'colorName': colorName,
                        'done': false,
  ...
  void _showEditTaskDialog(
    ...
    String? selectedCategory = categories.contains(currentCategory) ? currentCategory : 'Meds';
  ...
  String _getIconNameFromCategory(String category) {
    switch (category) {
      ...
      case 'Meds': return 'medication'; // <-- ESTABA MAL ESCRITO, DECÍA "MSeds"
